...
class Database {
  ...
  async getLayeredSearchProgress() {
    let { records, summary } = await this.driver.executeQuery(
      `
      MATCH (p:Page)
      RETURN p.depth AS depth, count(p) AS total, sum(CASE WHEN p.expanded THEN 1 ELSE 0 END) AS searched
      ORDER BY p.depth
      `,
      {},
      this.context
    ...
    return records.map(x => ({
      depth: x.get("depth").low, 
      total: x.get("total").low, 
      searched: x.get("searched").low
    }));
  }
}
